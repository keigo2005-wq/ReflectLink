...
F-14：行動計画・練習メニュー管理（課題整理後、次に取り組む行動や練習メニュー・期限を記録し、進捗（未着手／取り組み中／完了）を管理する）  
...
S-08：行動計画画面（次に取り組む行動・練習メニューを登録し、進捗を管理する）  
...
図９　行動計画画面（Java/Spring Bootの別サービスと連携）  
<img width="900" alt="行動計画画面" src="docs/screenshots/09_action_plan.png" />  
...
action_plans：行動計画・練習メニューを保存するテーブル（別サービスのJavaアプリが管理）  
...
| カラム名 | データ型 | 説明 |
|---|---|---|
| id | bigint(20) | 行動計画を一意に識別する番号 |
| post_id | int(11) | 対象となる投稿のID |
| action_text | text | 次に取り組む具体的な行動 |
| practice_menu | text | 取り入れる練習メニュー（任意） |
| due_date | date | いつまでに取り組むか（任意） |
| status | varchar(20) | 進捗状況（未着手・取り組み中・完了） |
| created_by_name | varchar(100) | 登録したアカウントの表示名 |
| created_at | datetime | 登録日時 |  
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td rowspan="2">行動計画サービス</td>
      <td>Java 21 / Spring Boot</td>
      <td>PHP本体とは別プロセスで動く、行動計画・練習メニュー管理サービス</td>
    </tr>
    <tr>
...
### 10.5.	PHPとJavaを分けてみた
　行動計画・練習メニュー機能は、PHP本体とは別に、Java（Spring Boot）で作った別サービスとして実装した。PHPの画面からは、直接データベースを触るのではなく、Javaが用意したAPI（決まった形式でのやり取り窓口）にリクエストを送って結果を受け取る形にしている。  
