...
課題・原因・改善案を投稿  
        ↓  
監督・コーチ・選手がポジション別にコメント  
        ↓  
統計・AIによる傾向分析で振り返りを確認  
        ↓  
改善状況を更新  
        ↓  
（日曜日）個人でその週の目標・取り組む行動を設定し、翌週の日曜日に自己評価を記録
...
F-09：統計表示（ポジション別のコメント件数と、投稿の改善状況の内訳をグラフで集計・表示する）  
F-10：CSV出力（投稿データをCSVファイルとしてダウンロードする）  
F-11：文字数カウント（入力中の文字数をリアルタイムで表示する）  
F-12：AIによる傾向分析（蓄積した投稿をAIがまとめて分析し、繰り返し発生している課題・局面別の傾向・改善アイデアに加え、次の練習で使える具体的な練習メニューまで提示する）  
F-13：外部連携用API（APIキーを発行し、外部のプログラムから投稿の取得・コメント投稿ができる）  
F-14：個人目標管理（毎週の振り返りをもとに、対象期間(1週間)ごとの目標・取り組む行動・自己評価を記録する。特定の投稿とは切り離した本人専用のページ）  
...
S-05：統計画面（ポジション別のコメント件数と投稿の改善状況をまとめて表示する）  
...
※APIキー設定画面へは、投稿一覧画面下部の目立たないテキストリンクから遷移する（主要な5つのボタンとは別導線）。  

### 5.3.	画面イメージ
（部員の氏名・対戦大学名など個人情報につながる欄は、すべて黒塗りにしている）
...
図６　統計画面  
<img width="900" alt="統計画面" src="docs/screenshots/06_statistics.png" />  
...
| period_start | date | 対象期間の開始日（通常は日曜日） |
| period_end | date | 対象期間の終了日（通常は翌週土曜日） |
| goal | text | 具体的な目標 |
...
統計：どの立場やポジションから意見が出ているか、改善状況の進み具合を把握できる。  
文字数カウント：入力状況を確認しながら、内容を適切な長さに整理できる。  
CSVダウンロード：蓄積した投稿を保存・共有し、表計算ソフトなどで整理できる。  

### 9.4.	色による一目でわかる状態表示
　局面（攻撃は赤・守備は青）や改善状況（未実施は赤・実践中はオレンジ・達成済みは緑）を文字色で区別し、一覧画面をざっと見るだけで状況を把握できるようにした。目標管理画面の進捗ステータスも同じ配色に揃え、アプリ全体で色の意味が一貫するようにしている。  

### 9.5.	意図しない編集を防ぐ目標管理画面
　目標管理画面では、登録済みの目標は最初は文字だけの表示にし、「編集する」ボタンを押した投稿だけが入力欄に切り替わる仕様にした。うっかり入力欄をクリックして内容を書き換えてしまう事故を防いでいる。  
...
　蓄積した投稿をそのままAIに渡すだけでは、ただの要約になってしまう。そこで、「①繰り返し起きている課題」「②攻撃・守備などの局面ごとの傾向」「③過去の投稿を組み合わせた新しい改善案」「④③を踏まえて次の練習で実際に使える具体的な練習メニュー」の4点を答えてもらうよう、AIへの依頼文（プロンプト）を具体的に設計した。特に④を加えたことで、分析結果を読んで終わりではなく、次の行動にそのままつなげられるようにした。これにより、複数の投稿をまたいで気づきを組み合わせた提案をAIが返すようになった。また開発中、使っていたAIのモデル（種類）が急に廃止される出来事があったため、モデルを固定で指定せず、常に最新版を自動で使う設定に変更し、今後モデルが変わっても困らないようにした。  
　なお、AIの回答はMarkdown（`#`や`*`で見出しや箇条書きを表す記法）で返ってくるため、そのまま表示すると記号がそのまま画面に出てしまう問題があった。見出し・箇条書き・太字を実際のHTMLタグに変換する簡単な処理を自作し、記号ではなく見た目として文字の大きさや強調を反映できるようにした。  
...
外部連携用のJSON REST APIの仕様は[API.md](API.md)を参照。
