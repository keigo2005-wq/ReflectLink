...
改善状況を更新  
        ↓  
個人でその週の目標・取り組む行動を設定し、翌週に自己評価を記録
...
F-14：個人目標管理（毎週の振り返りをもとに、1週間ごとの目標・取り組む行動・自己評価を記録する。特定の投稿とは切り離した本人専用のページ）  
...
　蓄積した投稿をそのままAIに渡すだけでは、ただの要約になってしまう。そこで、「①繰り返し起きている課題」「②攻撃・守備などの局面ごとの傾向」「③過去の投稿を組み合わせた新しい改善案」「④③を踏まえて次の練習で実際に使える具体的な練習メニュー」の4点を答えてもらうよう、AIへのプロンプトを具体的に設計した。特に④を加えたことで、分析結果を読んで終わりではなく、次の行動にそのままつなげられるようにした。これにより、複数の投稿をまたいで気づきを組み合わせた提案をAIが返すようになった。
...
　こうすることで、この機能側でエラーが起きても、投稿やログインなどPHPで作った他の機能には影響しない。異なる言語のプログラム同士を、APIでつなげる経験ができた。  
